...
#
# Idempotent: adds a [scrolling] block if missing, updates `history` if present,
...
if grep -qE '^\[scrolling\]' "$CONF"; then
  # Update history inside the existing [scrolling] block (first occurrence).
  awk -v h="$HISTORY" '
    /^\[scrolling\]/ { in_sb=1; print; next }
    /^\[/ && in_sb    { in_sb=0 }
  ...
  if ! awk '/^\[scrolling\]/{f=1} f&&/^[[:space:]]*history[[:space:]]*=/{ok=1} END{exit !ok}' "$CONF"; then
    printf '\n[scrolling]\nhistory = %s\n' "$HISTORY" >> "$CONF"
  fi
else
...
echo "Set [scrolling].history = $HISTORY in $CONF"
